...
CREATE INDEX IF NOT EXISTS idx_image_entity_created_at ON image_entity(created_at);
...
-- ✅ Product filters: 2-10x faster
-- ✅ Order queries: 3-8x faster
-- ✅ User lookups: 2-5x faster
-- ✅ Search queries: 5-15x faster
-- ✅ Soft delete queries: Optimized with partial indexes
--
-- Recommended:
-- 1. Monitor index usage after 1 week: SELECT * FROM pg_stat_user_indexes ORDER BY idx_scan DESC;
-- 2. Run VACUUM ANALYZE weekly for best performance
-- 3. Consider archiving old orders/soft-deleted data
--
-- ==============================================
