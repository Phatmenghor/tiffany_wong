    ...
    <div className="relative">
      ...
      {isOpen && (
        <div
          ...
          className={cn(
            "absolute top-full mt-[0.325rem] w-[10.4rem] bg-background border border-border rounded-[0.325rem] shadow-lg overflow-hidden z-50",
        ...
        >
          ...
          {header && (
            <>
              <div className="p-[0.4875rem]">{header}</div>
              <div className="h-px bg-border" />
            </>
          )}

          {/* Sections */}
          <div className="py-[0.325rem]">
              ...
              <div key={sectionIndex}>
                ...
                {section.items.map((item, itemIndex) => (
                  <div key={itemIndex}>
                    ...
                    >
                      {item.icon && (
                        <span className="mr-[0.4875rem] flex-shrink-0 text-muted-foreground">
                          {item.icon}
                        </span>
                      )}
                      <span
                    ...
                    {/* Separator */}
                    {item.separator && <div className="my-[0.1625rem] h-px bg-border" />}
                  </div>
                ))}

                {/* Section Separator */}
                {sectionIndex < sections.length - 1 && (
                  <div className="my-[0.1625rem] h-px bg-border" />
                )}
              </div>
            ))}
          </div>
