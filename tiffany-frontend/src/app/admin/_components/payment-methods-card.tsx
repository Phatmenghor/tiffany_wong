...
function PieTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-popover border rounded-[0.1625rem] shadow-lg px-[0.4875rem] py-[0.325rem] text-[11px]">
  ...
  return (
    <Card>
      <CardHeader className="pb-[0.1625rem]">
        <CardTitle className="text-[13px]">Payment Methods</CardTitle>
        <CardDescription className="text-[11px]">Revenue by payment type</CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? <ChartSkeleton height={240} /> : !payments?.data?.length ? (
          <div className="h-[240px] flex items-center justify-center text-muted-foreground text-[11px]">No payment data</div>
        ) : (
          <>
            ...
            <div className="mt-[0.1625rem] space-y-[0.1625rem]">
              {payments.data.map((item, i) => (
                <div key={item.method} className="flex items-center justify-between text-[11px]">
                  <div className="flex items-center gap-[0.325rem]">
                    <div className="w-[0.5rem] h-[0.5rem] rounded-full shrink-0" style={{ backgroundColor: COLORS[item.method] ?? `hsl(var(--chart-${(i % 5) + 1}))` }} />
                  ...
                  <div className="text-right">
                    ...
                    <span className="ml-[0.325rem] text-[11px] text-muted-foreground tabular-nums">{formatCurrency(item.amount)}</span>
                  </div>
                </div>
              ))}
            </div>
