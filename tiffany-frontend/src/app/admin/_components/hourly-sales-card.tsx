    ...
    <Card>
      <CardHeader className="pb-[0.1625rem]">
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-[13px]">Hourly Sales Pattern</CardTitle>
            <CardDescription className="text-[11px]">Today's revenue by hour (current: {formatHour(currentHour)})</CardDescription>
          </div>
          {hourlyData.length > 0 && (
            <Badge variant="outline" className="gap-[0.1625rem] text-[11px]">
              <Flame className="h-[0.65rem] w-[0.65rem] text-rose-500" />Peak: {formatHour(peakHour)}
            </Badge>
          )}
        </div>
      </CardHeader>
      <CardContent>
        {loading ? <ChartSkeleton height={200} /> : !hourlyData.length ? (
          <div className="h-[200px] flex items-center justify-center text-muted-foreground text-[11px]">No hourly data</div>
        ) : (
