...
export function LocationPrimaryBanner({ location }: LocationPrimaryBannerProps) {
  ...
  return (
    <div className="mb-5 rounded-2xl bg-gradient-to-r from-amber-50 to-amber-100/60 border border-amber-200/70 overflow-hidden shadow-sm">
      <div className="flex items-start gap-4 p-4 sm:p-5">
        {/* Icon */}
        <div className="p-3 rounded-xl bg-amber-100 shrink-0">
          <LabelIcon className="h-5 w-5 text-amber-600" />
        </div>
        ...
        <div className="hidden sm:flex items-center justify-center shrink-0">
          <MapPin className="h-10 w-10 text-amber-300/60" />
        </div>
      </div>
    </div>
  );
